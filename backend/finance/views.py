...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    queryset = (Account.objects.prefetch_related().annotate(
        transaction_count=Count("transaction")).order_by("-transaction_count"))
    ...
    def analytics(self, request, *args, **kwargs):
        ...
        categories = list(
            Category.objects.values_list("id", flat=True).order_by("id"))
        per_month = (qs.filter(started_at__year=year).annotate(
            month=TruncMonth("started_at")).values("month").annotate(
                **{
        ...
        years = list(
            map(
                attrgetter("year"),
                qs.annotate(year=TruncYear("started_at")).values_list(
                    "year", flat=True).distinct("year").order_by("year"),
            ))
        return JsonResponse(
            data={
                "per_month": [{
                ...
                "categories": [
                    ...
                    if sum(map(itemgetter(cat), per_month))
                ],
            })
...
class CreditViewSet(viewsets.ViewSet):
    ...
    def list(self, request, **kwargs):
        ...
        rates = (ExchangeRate.objects.filter(
            ...
            | Q(symbol__endswith=credit.currency)).distinct("symbol").order_by(
                "symbol", "-date"))
        return JsonResponse(
            data={
...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if search_term := params.get("search_term"):
            queryset = queryset.annotate(search=SearchVector(
                "description", "additional_data", "amount", "type",
                "started_at"), ).filter(search=search_term)
    ...
    def update_all(self, request, *args, **kwargs):
        ...
        total = queryset.update(
            category=category,
            category_suggestion_id=(None if category != Category.UNIDENTIFIED
                                    else F("category_suggestion_id")),
            confirmed_by=(Transaction.CONFIRMED_BY_ML
                          if category != Category.UNIDENTIFIED else
                          Transaction.CONFIRMED_BY_UNCONFIRMED),
        )
        response = self.list(request, *args, **kwargs)
        response.data["msg"] = {
    ...
    def _populate_filters(self, response):
        response.data["types"] = (Transaction.objects.filter(
            amount__lt=0).values_list(
                "type", flat=True).distinct("type").order_by("type"))
        response.data[
            "confirmed_by_choices"] = Transaction.CONFIRMED_BY_CHOICES
        response.data["categories"] = Category.objects.values_list(
            "id", flat=True).order_by("id")
        response.data["accounts"] = Account.objects.values(
            "id", "bank", "type")
        response.data["unidentified_count"] = (
            Transaction.objects.expenses().filter(
                category=Category.UNIDENTIFIED, ).count())
        return response
...
class PredictionViewSet(viewsets.ViewSet):
    ...
    def start_prediction(self, request, *args, **kwargs):
        ...
        if (status := details.get("status")) and status not in FINAL_STATUSES:
            return JsonResponse({"error": f"prediction - {status}"},
                                status=400)
        ...
        return JsonResponse(data={
            "type": "predict",
            **log_status("predict", status="initial")
        })
    ...
    def start_training(self, request, *args, **kwargs):
        ...
        return JsonResponse(data={
            "type": "train",
            **log_status("train", status="initial")
        })
