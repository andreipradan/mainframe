...
class TestAccounts:
    ...
    def test_list(self, client, django_assert_num_queries, session):
        ...
        with django_assert_num_queries(4):
            response = client.get(reverse("finance:accounts-list"),
                                  HTTP_AUTHORIZATION=session.token)
        assert response.status_code == 200
        assert set(response.json().keys()) == {
            "count", "next", "previous", "results"
        }
        assert [(x["id"], x["transaction_count"])
                for x in response.json()["results"]] == [(account1.id, 2),
                                                         (account2.id, 1),
                                                         (account3.id, 0)]
...
class TestCredit:
    ...
    def test_list(self, client, django_assert_num_queries, session):
        ...
        with django_assert_num_queries(5):
            response = client.get(reverse("finance:credit-list"),
                                  HTTP_AUTHORIZATION=session.token)
        assert response.status_code == 200
        assert set(
            response.json().keys()) == {"credit", "latest_timetable", "rates"}
...
class TestPayments:
    ...
    def test_payment_list(self, client, django_assert_num_queries, session):
        ...
        with django_assert_num_queries(4):
            response = client.get(reverse("finance:payments-list"),
                                  HTTP_AUTHORIZATION=session.token)
        assert response.status_code == 200
