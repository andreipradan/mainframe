...
def download_blob(blob_name, destination_path):
    ...
    bucket.blob(blob_name).download_to_filename(
        f"{destination_path}/{blob_name}")


def download_blob_into_memory(blob_name, bucket_var=None):
    storage_client = storage.Client()
    bucket = storage_client.bucket(
        config(bucket_var or "GOOGLE_STORAGE_BUCKET"))
...
class CameraViewSet(viewsets.ViewSet):
    ...
    def list(self, request, **kwargs):
        ...
        if not prefix:
            ...
            Path(f"{self.base_path}/{prefix}").mkdir(parents=True,
                                                     exist_ok=True)
        blobs = list_blobs_with_prefix(f"{prefix}/" if prefix else None)
        local_files = list(
            map(itemgetter("name"),
                get_folder_contents(self.base_path / prefix)))
        files = [{
            "name": b.name.split("/")[-1],
            "is_file": True,
    ...
    def create_folder(self, request):
        ...
        if not folder:
            return JsonResponse(status=400,
                                data={"error": "Missing folder name"})
