...
from credit.tests.factories import PaymentFactory, CreditFactory
...
class TestOverview:
    def test_overview(self, client, django_assert_num_queries, token):
        ...
        with django_assert_num_queries(5):
            response = client.get(
                reverse("credit:overview-list"), HTTP_AUTHORIZATION=token
            )
        assert response.status_code == 200
        assert set(response.json().keys()) == {"credit", "latest_timetable", "payments"}
...
class TestPayments:
    def test_payment_list(self, client, django_assert_num_queries, token):
        ...
        with django_assert_num_queries(4):
            response = client.get(
                reverse("credit:payments-list"), HTTP_AUTHORIZATION=token
            )
        assert response.status_code == 200
