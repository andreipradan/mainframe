  ...
  class extends Base {
    constructor(token) {
      ...
      if (this.constructor.baseUrl === undefined) {
        throw new Error(
          `${this.constructor.name} class must implement the static 'baseUrl' attribute`
        );
      }
      if (this.constructor.methods === undefined) {
        throw new Error(
          `${this.constructor.name} class must implement the static 'methods' object`
        );
      }
      ['set', 'setErrors', 'setLoading'].forEach((method) => {
        if (typeof this.constructor.methods[method] !== 'function') {
          throw new TypeError(
            `'${method}' is missing from the ${this.constructor.name}.methods object.`
          );
        }
      });
    }
...
export const CreateApi = (Base) =>
  class extends Base {
    ...
    create = (data) => (dispatch) => {
      ...
      axios
        .post(`${this.constructor.baseUrl}/`, data, {
        ...
        .then((response) => {
          ...
          toast.success(
            `${getResource(this.constructor.baseUrl)} ${displayField} created successfully!`,
            toastParams
          );
        })
        .catch((err) =>
          handleErrors(
            err,
            dispatch,
            this.constructor.methods.setErrors,
            this.constructor.methods.setLoading
          )
        );
    };
  };

export const DeleteApi = (Base) =>
  class extends Base {
    constructor(token) {
      super(token);
      if (typeof this.constructor.methods.delete !== 'function') {
        throw new TypeError(
          `'delete' is missing from the ${this.constructor.name}.methods object`
        );
      }
    ...
    delete =
      ...
      (dispatch) => {
        ...
        axios
          .delete(`${this.constructor.baseUrl}/${id}/`, {
            headers: { Authorization: this.token },
          })
          .then(() => {
...
export const DetailApi = (Base) =>
  class extends Base {
    ...
    getItem = (id) => (dispatch) => {
      dispatch(this.constructor.methods.setLoadingItems(id));
      axios
        .get(`${this.constructor.baseUrl}/${id}/`, {
          headers: { Authorization: this.token },
        })
        .then((response) =>
          dispatch(
            this.constructor.methods.update({
              ...response.data,
              dontClearSelectedItem: true,
            })
          )
        )
        .catch((err) => {
  ...
  class extends Base {
    getList =
      ...
      (dispatch) => {
        ...
        axios
          .get(`${this.constructor.baseUrl}/?${createSearchParams(kwargs)}`, {
            headers: { Authorization: this.token },
          })
          .then((response) =>
...
export const RunApi = (Base) =>
  class extends Base {
    run =
      (id, data, verbose = null) =>
      (dispatch) => {
        dispatch(this.constructor.methods.setLoading());
        const axios = getAxios('run', this.constructor.ngrokAxios);
        axios
          .put(`${this.constructor.baseUrl}/${id}/run/`, data, {
            headers: { Authorization: this.token },
          })
          .then(() => {
...
export const UpdateApi = (Base) =>
  class extends Base {
    ...
    update = (id, data) => (dispatch) => {
      dispatch(this.constructor.methods.setLoadingItems(id));
      axios
        .patch(`${this.constructor.baseUrl}/${id}/`, data, {
          headers: { Authorization: this.token },
        })
        .then((response) => {
          dispatch(
          ...
          const displayField = getDisplayName(this.constructor, response.data);
          toast.info(
            `${getResource(this.constructor.baseUrl)} "${displayField}" updated successfully!`,
            toastParams
          );
        })
        .catch((err) =>
          handleErrors(
            ...
            dispatch,
            this.constructor.methods.setErrors,
            this.constructor.methods.setLoadingItems
          )
        );
    };
  };

export const UploadApi = (Base) =>
  class extends Base {
    upload = (data) => (dispatch) => {
      ...
      axios
        ...
        .then((response) => {
          dispatch(this.constructor.methods.set(response.data));
          toast.success(
            `${getResource(this.constructor.baseUrl)} uploaded successfully!`,
            toastParams
          );
        })
        .catch((err) =>
