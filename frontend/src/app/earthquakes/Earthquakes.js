...
const Earthquakes = () => {
  ...
  const earthquakesList = earthquakes.results
    ? [...earthquakes.results].reverse()
    : null;
  ...
  useEffect(() => {
    setAlertOpen(!!earthquakes.errors);
  }, [earthquakes.errors]);
    ...
    datasets: [
      {
        ...
        backgroundColor: (context) => {
          if (
            earthquakesList?.[context.dataIndex]?.additional_data?.sols?.primary
              ?.region?.type === 'world'
          )
  ...
  useLayoutEffect(() => {
    ...
    const countySeries = chart.series.push(
          ...
          <div className='card'>
            <div className='card-body'>
                  ...
                  <label className='form-check-label text-muted'>
                    <input
                      ...
                      onChange={() =>
                        dispatch(
                          setKwargs({
                            local_events: !earthquakes.kwargs.local_events,
                          })
                        )
                      }
                      checked={earthquakes.kwargs.local_events}
                    />
                  ...
                  <label className='form-check-label text-muted'>
                    <input
                      ...
                      onChange={() =>
                        dispatch(
                          setKwargs({
                            magnitude_gt5: !earthquakes.kwargs.magnitude_gt5,
                          })
                        )
                      }
                      checked={earthquakes.kwargs.magnitude_gt5}
                    />
                    <i className='input-helper' />
                    &gt; 5 ML
                  </label>
                  ...
                  <label className='form-check-label text-muted'>
                    <input
                      ...
                      onChange={() =>
                        dispatch(
                          setKwargs({
                            largest_events: !earthquakes.kwargs.largest_events,
                          })
                        )
                      }
                      checked={earthquakes.kwargs.largest_events}
                    />
                    <i className='input-helper' />
                    Largest events
                  </label>
                ...
                ? Object.keys(earthquakes.last_check).map((lc) => (
                    <p key={lc} className='text-muted mt-0 mb-0'>
                      ...
                      {earthquakes.loading ? (
                        ...
                        <span>
                          {' '}
                          {earthquakes.last_check[lc]
                            ? formatTime(earthquakes.last_check[lc])
                            : '-'}
                        </span>
                      )}
                    </p>
              ...
              <p className='text-muted mb-0'>
                Total: {earthquakes.count ? earthquakes.count : '-'}
              </p>
                ...
                <div className='col-md-12'>
                  {earthquakes.loading ? (
                    ...
                    <Bar
                      ...
                      onElementsClick={(e) =>
                        e?.[0]?._index &&
                        zoomToGeoPoint(earthquakesList?.[e[0]._index])
                      }
                    />
                  )}
                </div>
                ...
                <div className='col-md-7'>
                  ...
                  >
                    <table className='table'>
                      ...
                      >
                        {earthquakes.results?.map((e, i) => (
                          <tr key={i} onClick={() => zoomToGeoPoint(e)}>
                            <td>
                            ...
                            <td>
                              <a
                                ...
                                target='_blank'
                                href={`https://www.google.com/maps/place/${e.latitude}+${e.longitude}`}
                              >
                            ...
                            <td className='font-weight-medium'>
                              {' '}
                              {parseFloat(e.depth).toFixed(2)} km{' '}
                            </td>
                            <td className='font-weight-medium'>
                              {' '}
                              {e.location}{' '}
