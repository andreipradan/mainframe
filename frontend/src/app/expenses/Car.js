...
const Car = () => {
  ...
  const onSubmit = useCallback(
    (e) => {
      e.preventDefault();
      const data = { name };
      if (store.selectedItem) dispatch(api.update(store.selectedItem.id, data));
  ...
  return (
    <div>
            ...
            <div className='card-body'>
              ...
              <div className={'row'}>
                {store.loading ? (
                  ...
                  store.results.map((p) => (
                    <div className='col grid-margin' key={`car-${p.id}`}>
                      <h6>
                        <button
                          type='button'
                      ...
                      <div className='col-sm-12'>
                        <h6 className='text-secondary'>
                          ...
                          <button
                            ...
                            onClick={() =>
                              setServiceEntryModalOpen(
                                serviceEntryModalOpen === p.id ? null : p.id
                              )
                            }
                          >
                            <i className='mdi mdi-plus' />
                          ...
                          <p className={'text-muted'}>
                            Service entries: {p.service_entries?.length || 0}
                            <br />
                            Total spent:{' '}
                            {p.service_entries?.length
                              ? p.service_entries
                        ...
                        <Collapse in={serviceEntryModalOpen === p.id}>
                          <ul className='navbar-nav w-100 rounded'>
                            <li className='nav-item w-100'>
                              <Form
                                onSubmit={(e) => onSubmitServiceEntry(e, p.id)}
                              >
                                <div className='row'>
                                    ...
                                    <Form.Group className='row'>
                                      ...
                                      <div className='col-sm-8'>
                                        <Form.Control
                                          ...
                                          onChange={(e) =>
                                            setServiceEntryPrice(e.target.value)
                                          }
                                        />
                                      </div>
                                    </Form.Group>
                                ...
                                <Button
                                  ...
                                  disabled={
                                    !serviceEntryDate || !serviceEntryPrice
                                  }
                                >
                                  Add
                                </Button>
                  ...
                  <table className='table table-hover'>
                    ...
                    <tbody>
                      {store.loading ? (
                        ...
                        p.service_entries.map((c) => (
                          <tr key={`service-entry-${p.id}-${c.id}`}>
                            <td>
                              {' '}
                            ...
                            <td>
                              <i
                                ...
                                onClick={() =>
                                  dispatch(api.deleteServiceEntry(p.id, c.id))
                                }
                              />
                            </td>
                          </tr>
                        ...
                        <tr>
                          <td colSpan={6}>
                            <span>No service entries found</span>
                          </td>
                        </tr>
                      )}
                    </tbody>
      ...
      <Modal
        centered
        show={Boolean(store.selectedItem) || store.modalOpen}
        onHide={closeModal}
      >
          ...
          <Modal.Title>
            <div className='row'>
              <div className='col-lg-12 grid-margin stretch-card'>
                {store.selectedItem ? 'Edit' : 'Add new'}{' '}
        ...
        <Modal.Footer>
          {Boolean(store.selectedItem) && (
            <Button
              ...
              onClick={() =>
                dispatch(
                  api.delete(store.selectedItem.id, store.selectedItem.name)
                )
              }
            >
              Delete
