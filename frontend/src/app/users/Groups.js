...
const Groups = () => {
  ...
  const toggleEmailOpen = (groupId) => {
    emailOpen === groupId ? setEmailOpen(null) : setEmailOpen(groupId);
  };
  ...
  return (
    <div>
      ...
      {(groups.errors?.detail ||
        groups.errors?.length ||
        groups.errors?.name) &&
      (!groupToRemove || !userToRemove || !addGroupOpen) ? (
                                ...
                                {t.users.map((user) => (
                                  <li key={user.id}>
                                    {t.created_by === user.id ? (
                                      <small>
                                        <i className='text-warning mdi mdi-star' />
                                        &nbsp;
                                      </small>
                                    ) : null}
      ...
      <Modal centered show={addGroupOpen} onHide={() => setAddGroupOpen(false)}>
        ...
        <Modal.Body>
          ...
          >
            {(groups.errors?.detail || groups.errors?.length) &&
            addGroupOpen ? (
              <Errors errors={groups.errors} />
            ) : null}
            <Form.Group>
      ...
      <Modal
        centered
        show={Boolean(groupToRemove)}
        onHide={() => setGroupToRemove(null)}
      >
        <Modal.Header closeButton>
          <Modal.Title>
            <div className='row'>
            ...
            <p className='text-muted mb-0'>
              <ul>
                <li>
                  Group:{' '}
                  <span className='text-danger'>{groupToRemove?.name}</span>
                </li>
                <li>
                  Users:
                  <ul>
                    {groupToRemove?.users?.length
                      ? groupToRemove?.users?.map((u) => (
                          <li key={u.id}>
                            {u.username} ({u.email})
                          </li>
                        ))
        ...
        <Modal.Footer>
          <Button variant='success' onClick={() => setGroupToRemove(null)}>
            No, go back
          </Button>
          <Button
            variant='danger'
      ...
      <Modal centered show={userToRemove} onHide={() => setUserToRemove(null)}>
          ...
          <Modal.Title>
            ...
            <p className='text-muted mb-0'>
              <ul>
                ...
                <li>
                  User: {userToRemove?.username} ({userToRemove?.email})
                </li>
              </ul>
            </p>
          </Modal.Title>
        ...
        <Modal.Body>
          Remove user <b>{userToRemove?.username}</b> from group{' '}
          <b>{groups.selectedItem?.name}</b>?
        </Modal.Body>
        <Modal.Footer>
          <Button variant='success' onClick={() => setUserToRemove(null)}>
            No, go back
          </Button>
          <Button
            variant='danger'
