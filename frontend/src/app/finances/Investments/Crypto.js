...
const Crypto = () => {
  ...
  const [selectedTransactionsFile, setSelectedTransactionsFile] =
    useState(null);
  ...
  const onChange = (what, newValue, store) => {
    ...
    let setKwargsMethod;
    if (store === crypto) setKwargsMethod = setKwargs;
    else if (store === pnl) setKwargsMethod = setPnlKwargs;
    else throw Error('Invalid store');
    dispatch(
      setKwargsMethod({ ...(store.kwargs || {}), [what]: newTypes, page: 1 })
    );
  };

  useEffect(() => setUploadPnlOpen(Boolean(pnl.errors)), [pnl.errors]);
  useEffect(
    () => setUploadTransactionsOpen(Boolean(crypto.errors)),
    [crypto.errors]
  );
  ...
  return (
    <div>
      ...
      <div className='row'>
        <div
          className={`col-sm-${crypto.loading && !crypto.results ? 12 : 4} grid-margin`}
        >
          <div className='card'>
            <div className='card-body'>
              ...
              {!crypto.loading && crypto.aggregations && (
          ...
          <div className='col-sm-4 grid-margin' key={currency}>
            <div className='card'>
              <div className='card-body'>
                ...
                <div style={{ maxHeight: '22vh', overflowY: 'scroll' }}>
                  {!crypto.loading &&
                    crypto.aggregations?.[currency].totals.map((item, i) => (
                      <ListItem
                        key={item.type}
                        label={capitalize(item.type.replace('_', ' '))}
                        value={
                          item.value
                            ? `${currency === 'USD' ? '$ ' : currency === 'EUR' ? '€ ' : ''}${item.value} (${crypto.aggregations?.[currency].counts[i + 1].value})${['USD', 'EUR'].includes(currency) ? '' : ` ${currency}`}`
                            : '-'
                        }
                        textType={'warning'}
                      />
            ...
            <div className='card-body'>
              ...
              {crypto.loading ? (
                ...
                <Marquee duration={10000} pauseOnHover>
                  <ListItem
                    label={'Date'}
                    value={new Date(crypto.results[0].date).toLocaleString()}
                    textType={'warning'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Type'}
                    value={crypto.results[0].type}
                    textType={'warning'}
                    className='mr-3'
                  />
                  {crypto.results[0].quantity && (
                    <ListItem
                      label={'Quantity'}
                      value={crypto.results[0].quantity}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {crypto.results[0].symbol && (
                    <ListItem
                      label={'Ticker'}
                      value={crypto.results[0].symbol}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {crypto.results[0].price && (
                    <ListItem
                      label={'Price'}
                      value={crypto.results[0].price}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {crypto.results[0].fees && (
                    <ListItem
                      label={'Price'}
                      value={crypto.results[0].fees}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {crypto.results[0].value && (
                    <ListItem
                      label={'Amount'}
                      value={`${crypto.results[0].currency === 'USD' ? '$ ' : crypto.results[0].currency === 'EUR' ? '€ ' : ''}${crypto.results[0].value}${['USD', 'EUR'].includes(crypto.results[0].currency) ? '' : ` ${crypto.results[0].currency}`}`}
                      textType={'success'}
                      className='mr-3'
                    />
                  )}
            ...
            <div className='card-body'>
              ...
              <div className='table-responsive'>
                <div className='mb-0 text-muted'>
                  <div className='row'>
                    <div
                      className={`col-sm-${pnl.currencies?.length > 1 ? '6' : '9'}`}
                    >
                    ...
                    <div
                      className={`col-sm-${pnl.currencies?.length > 1 ? '6' : '3'}`}
                    >
                      <Form
                        className='row'
                        onSubmit={(e) => {
                      ...
                      >
                        {pnl.currencies?.length > 1 ? (
                          <Form.Group className='col-md-6'>
                            ...
                            <Select
                              ...
                              options={pnl.currencies?.map((t) => ({
                                label: t,
                                value: t,
                              }))}
                              styles={selectStyles}
                              value={pnl.kwargs.currency?.map((t) => ({
                                label: t,
                                value: t,
                              }))}
                            />
                        ...
                        <Form.Group
                          className={`col-sm-${pnl.currencies?.length > 1 ? '6' : '12'}`}
                        >
                          ...
                          <Select
                            ...
                            options={pnl.tickers?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                            styles={selectStyles}
                            value={pnl.kwargs.ticker?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                          />
                    ...
                    {pnl.loading ? (
                      ...
                      pnl.results.map((transaction) => (
                        <tr key={transaction.id}>
                          ...
                          <td
                            className={`text-${transaction.gross_pnl < 0 ? 'danger' : 'success'}`}
                          >
                            {' '}
                            {transaction.currency === 'USD' ? '$' : '€'}
                          ...
                          <td
                            className={`text-${transaction.fees > 0 ? 'danger' : 'success'}`}
                          >
                            {' '}
                            {transaction.currency === 'USD' ? '$' : '€'}
                            {transaction.fees}{' '}
                          ...
                          <td
                            className={`text-${transaction.net_pnl < 0 ? 'danger' : 'success'}`}
                          >
                            {' '}
                            {transaction.currency === 'USD' ? '$' : '€'}
                            {transaction.net_pnl}{' '}
              ...
              <BottomPagination
                items={pnl}
                fetchMethod={pnlApi.getList}
                newApi
                setKwargs={setPnlKwargs}
              />
            </div>
          </div>
        </div>
            ...
            <div className='card-body'>
              <h4 className='card-title'>
                ...
                <button
                  ...
                  className='btn btn-outline-primary btn-sm border-0 bg-transparent float-right'
                  onClick={() =>
                    setUploadTransactionsOpen(!uploadTransactionsOpen)
                  }
                >
                  <i className='mdi mdi-upload' />
                </button>
                <Collapse in={uploadTransactionsOpen}>
                  ...
                  >
                    <Form.Group>
                      {transactionsFileError ? (
                        <Errors errors={[transactionsFileError]} />
                      ) : null}
                      <div className='custom-file'>
                        <Form.Control
                          type='file'
                        ...
                        <label
                          className='custom-file-label'
                          htmlFor='transactionsFile'
                        >
                          {selectedTransactionsFile
                            ? selectedTransactionsFile.name
                            : 'Select a file'}
                        </label>
                      </div>
                    </Form.Group>
                      ...
                      {Object.keys(crypto.kwargs).length ? (
                        <small className='text-warning'>
                          ...
                          {crypto.currencies
                            ?.map(
                              (c) =>
                                `${c === 'USD' ? '$' : c === 'EUR' ? '€' : c} ${crypto.aggregations?.current[`total_${c}`]}`
                            )
                      ...
                      >
                        <Form.Group className='col-md-4'>
                          ...
                          <Select
                            ...
                            options={crypto.currencies?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                            styles={selectStyles}
                            value={crypto.kwargs.currency?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                          />
                        ...
                        <Form.Group className='col-md-4'>
                          ...
                          <Select
                            ...
                            options={crypto.symbols?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                            styles={selectStyles}
                            value={crypto.kwargs.ticker?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                          />
                ...
                <table className='table table-hover'>
                  ...
                  <tbody>
                    {crypto.loading ? (
                      ...
                      crypto.results.map((transaction) => (
                        <tr key={transaction.id}>
                          <td>
                            {' '}
                          ...
                          <td
                            ...
                            onClick={() =>
                              onChange('type', [
                                {
                                  value: crypto.types.find(
                                    (t) => t[1] === transaction.type
                                  )[0],
                                },
                              ])
                            }
                          ...
                          <td>
                            ...
                            {transaction.currency === 'USD'
                              ...
                              : transaction.currency === 'EUR'
                                ...
                                : ''}
                            {transaction.value}
                            {['USD', 'EUR'].includes(transaction.currency)
                              ? ''
