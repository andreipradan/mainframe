...
const Stocks = () => {
  ...
  const onCurrencyChange = (newValue, store) => {
    ...
    dispatch(
      setKwargsMethod({ ...(store.kwargs || {}), currency: newTypes, page: 1 })
    );
  };
  ...
  const onTickerChange = (newValue, store = stocks) => {
    ...
    const newTypes = newValue.map((v) => v.value);
    dispatch(
      setKwargsMethod({ ...(store.kwargs || {}), ticker: newTypes, page: 1 })
    );
  };
  ...
  const onTypeChange = (newValue, store) => {
    ...
    const newTypes = newValue.map((v) => v.value);
    dispatch(
      setKwargsMethod({ ...(store.kwargs || {}), type: newTypes, page: 1 })
    );
  };
  ...
  return (
    <div>
      ...
      <div className='row'>
        <div
          className={`col-sm-${stocks.loading && !stocks.results ? 12 : 4} grid-margin`}
        >
          <div className='card'>
            <div className='card-body'>
              ...
              {!stocks.loading && stocks.aggregations && (
                <div style={{ maxHeight: '22vh', overflowY: 'scroll' }}>
                  <ListItem
                    label={'Total'}
                    value={calculateSum(
                      stocks.aggregations.quantities,
                      'value'
                    )}
                    textType={'warning'}
                    className='mr-3'
                  ...
                  {!stocks.loading &&
                    stocks.aggregations?.[currency].totals.map((item, i) => (
                      <ListItem
                        ...
                        value={
                          item.value
                            ? `${currency === 'USD' ? '$' : '€'} ${item.value} (${stocks.aggregations?.[currency].counts[i + 1].value})`
                            : '-'
                        }
                        textType={'warning'}
                      />
            ...
            <div className='card-body'>
              ...
              {stocks.loading ? (
                ...
                <Marquee duration={10000} pauseOnHover>
                  <ListItem
                    label={'Date'}
                    value={new Date(stocks.results[0].date).toLocaleString()}
                    textType={'warning'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Type'}
                    value={stocks.results[0].type}
                    textType={'warning'}
                    className='mr-3'
                  />
                  {stocks.results[0].quantity && (
                    <ListItem
                      label={'Quantity'}
                      value={stocks.results[0].quantity}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {stocks.results[0].ticker && (
                    <ListItem
                      label={'Ticker'}
                      value={stocks.results[0].ticker}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {stocks.results[0].price_per_share && (
                    <ListItem
                      label={'Price / Share'}
                      value={stocks.results[0].price_per_share}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  <ListItem
                    label={'Amount'}
                    value={`${stocks.results[0].currency === 'USD' ? '$' : '€'} ${stocks.results[0].total_amount}`}
                    textType={'success'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'FX Rate'}
                    value={stocks.results[0].fx_rate}
                    textType={'warning'}
                    className='mr-3'
                  />
                </Marquee>
                    ...
                    {pnl.loading ? (
                      ...
                      pnl.results.map((transaction) => (
                        <tr key={transaction.id}>
                          ...
                          <td
                            className={`text-${transaction.pnl < 0 ? 'danger' : 'success'}`}
                          >
                            {' '}
                            {transaction.pnl}{' '}
                          </td>
                      ...
                      {Object.keys(stocks.kwargs).length ? (
                        <small className='text-warning'>
                          ...
                          {stocks.currencies
                            ?.map(
                              (c) =>
                                `${c === 'USD' ? '$' : '€'} ${stocks.aggregations?.current[`total_${c}`]}`
                            )
                      ...
                      >
                        <Form.Group className='col-md-4'>
                          ...
                          <Select
                            ...
                            options={stocks.currencies?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                            styles={selectStyles}
                            value={stocks.kwargs.currency?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                          />
                        ...
                        <Form.Group className='col-md-4'>
                          ...
                          <Select
                            ...
                            options={stocks.tickers?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                            styles={selectStyles}
                            value={stocks.kwargs.ticker?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                          />
                ...
                <table className='table table-hover'>
                  ...
                  <tbody>
                    {stocks.loading ? (
                      ...
                      stocks.results.map((transaction) => (
                        <tr key={transaction.id}>
                          <td>
                            {' '}
                          ...
                          <td
                            style={{ cursor: 'pointer' }}
                            onClick={() =>
                              onTickerChange([{ value: transaction.ticker }])
                            }
                          >
                            {' '}
                            {transaction.ticker}{' '}
