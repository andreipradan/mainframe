...
const Bonds = () => {
  ...
  const onActiveOnlyChange = () => {
    ...
    dispatch(
      setKwargs({ ...(state.kwargs || {}), active_only: !activeOnlyChecked })
    );
  };
  ...
  const onSubmit = (e) => {
    ...
    if (interestDates)
      data.interest_dates = interestDates.map(
  ...
  useEffect(() => {
    if (state.selectedItem) {
      ...
      if (state.selectedItem.interest) setInterest(state.selectedItem.interest);
      if (state.selectedItem.interest_dates)
        setInterestDates(
          state.selectedItem.interest_dates.map((d) => new Date(d))
        );
                  ...
                  {state.aggregations
                    ? state.currencies?.map((currency) => (
                        <div key={currency}>
                          ...
                          {state.aggregations[`pnl_${currency}`] &&
                          state.aggregations[`dividend_${currency}`] ? (
                            <ListItem
                              label={'Profit'}
            ...
            <div className='card-body'>
              ...
              {state.loading ? (
                ...
                <Marquee duration={10000} pauseOnHover>
                  <ListItem
                    label={'Date'}
                    value={formatTime(state.results[0].date)}
                    textType={'warning'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Type'}
                    value={
                      state.types.find((t) => t[0] === state.results[0].type)[1]
                    }
                    textType={'warning'}
                    className='mr-3'
                  />
                  {state.results[0].quantity && (
                    <ListItem
                      label={'Quantity'}
                      value={state.results[0].quantity}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {state.results[0].ticker && (
                    <ListItem
                      label={'Ticker'}
                      value={state.results[0].ticker}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {state.results[0].price && (
                    <ListItem
                      label={'Price'}
                      value={state.results[0].price}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  {state.results[0].net && (
                    <ListItem
                      label={'Net'}
                      value={`${state.results[0].net} ${state.results[0].currency ? state.results[0].currency : ''}`}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  ...
                  {state.results[0].interest && (
                    <ListItem
                      label={'Interest'}
                      value={state.results[0].interest}
                      textType={'success'}
                      className='mr-3'
                    />
                  )}
                  {state.results[0].interest_dates && (
                    <ListItem
                      label={'Interest Dates'}
                      value={state.results[0].interest_dates
                        .map((d) => formatDate(d))
                        .join(', ')}
                      textType={'success'}
                      className='mr-3'
                      ...
                      >
                        ...
                        <Form.Group className='col-md-6'>
                          ...
                          <Select
                            ...
                            options={state.tickers?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                            styles={selectStyles}
                            value={state.kwargs.ticker?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                          />
                    ...
                    {state.loading ? (
                      ...
                      state.results.map((bond) => (
                        ...
                        >
                          ...
                          <td>
                            {state.types?.find((t) => t[0] === bond.type)[1]}
                          </td>
                          <td> {bond.ticker} </td>
                          ...
                          <td>
                            {bond.interest ? (
                              <span>
                                <span
                                  className={
                                    bond.interest ? 'text-success' : ''
                                  }
                                >
                                  {parseFloat(
                                    (bond.interest / 100) * -bond.net
                                  ).toFixed(2)}
                                  &nbsp;
                                  {bond.currency}&nbsp;
                                </span>
                          ...
                          <td
                            className={`text-${bond.pnl ? (bond.pnl < 0 ? 'danger' : 'success') : 'muted'}`}
                          >
                            {bond.pnl ? parseFloat(bond.pnl) : '-'}
                          </td>
                          <td>
                            {bond.interest_dates?.map((r) => (
                              <div key={r}>{formatDate(r)}</div>
                            ))}
                          </td>
          ...
          <Modal.Title>
            <div className='row'>
              <div className='col-lg-12 grid-margin stretch-card'>
                {state.selectedItem ? 'Edit' : 'Add new transaction'}{' '}
                {state.selectedItem?.ticker} ?
                <button
                  type='button'
                  className='btn btn-outline-success btn-sm border-0 bg-transparent'
            ...
            {state.selectedItem ? (
              ...
              <p className='text-muted mb-0'>
                Fill in the bond transaction details
              </p>
            )}
            ...
            <Form onSubmit={onSubmit}>
              ...
              <Form.Group className='col-md-4'>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setType({
                      label: state.types.find((t) => t[0] === e.value)[1],
                      value: e.value,
                    })
                  }
                  options={state.types?.map((t) => ({
              ...
              onClick={() =>
                dispatch(
                  api.delete(
                    ...
                    `${state.selectedItem.date} / ${state.selectedItem.ticker}`
                  )
                )
              }
            >
