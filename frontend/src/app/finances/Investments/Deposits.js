        ...
        <b className={'text-small text-muted'}>
          ...
          <ul className={'list-arrow'}>
            {state.currencies?.map((currency) => (
              <li className={'pt-0'} key={currency}>
              ...
              {state.loading ? (
                ...
                <Marquee duration={10000} pauseOnHover>
                  ...
                  <ListItem
                    ...
                    className='mr-3'
                  />
                  {state.results[0].amount && (
                    <ListItem
                      label={'Quantity'}
                      value={state.results[0].amount}
                      textType={'warning'}
                      className='mr-3'
                    />
                  )}
                  <ListItem
                    ...
                    value={
                      state.currencies?.find(
                        (c) => c[0] === state.results[0].currency
                      )?.[1] || state.results[0].currency
                    }
                    textType={'warning'}
                    className='mr-3'
                  />
                  ...
                  <ListItem
                    label={'Maturity'}
                    value={formatTime(state.results[0].maturity)}
                    textType={'warning'}
                    className='mr-3'
                  />
                  {state.results[0].pnl && (
                      ...
                      >
                        <Form.Group className='col-md-12'>
                          ...
                          <Select
                            ...
                            options={state.currencies?.map((t) => ({
                              label: t,
                              value: t,
                            }))}
                            styles={selectStyles}
                        ...
                        >
                          <td>
                            ...
                            {new Date(deposit.maturity) < new Date() ? (
                              <i
                                className={
                                  'text-success mdi mdi-check-circle-outline'
                                }
                              />
                            ) : null}
                            <br />
                            <sub>
                              {diffMonths(deposit.date, deposit.maturity)}{' '}
                              months
                            </sub>
                          </td>
                          ...
                          <td className={'text-warning'}>
                            {' '}
                            {parseFloat(deposit.amount)} {deposit.currency}
                          </td>
                          <td>
      ...
      >
          ...
          <Modal.Title>
            <div className='row'>
              <div className='col-lg-12 grid-margin stretch-card'>
                {state.selectedItem ? 'Edit' : 'Add new deposit'}{' '}
                ...
                {state.selectedItem ? (
                  <button
                    ...
                    className='btn btn-outline-success btn-sm border-0 bg-transparent'
                    onClick={() =>
                      dispatch(api.getItem(state.selectedItem?.id))
                    }
                  >
                    <i className='mdi mdi-refresh' />
                  </button>
            ...
            <Form onSubmit={onSubmit}>
              ...
              <Form.Group className='col-md-4'>
                ...
                <Select
                  ...
                  isLoading={state.loading}
                  onChange={(e) =>
                    setCurrency({ label: e.value, value: e.value })
                  }
                  options={state.currencies?.map((t) => ({
                    label: t,
                    value: t,
        ...
        <Modal.Footer>
          ...
          <Button
            ...
            onClick={onSubmit}
            disabled={
              !date || !maturity || !amount || !interest || !name || !currency
            }
          >
