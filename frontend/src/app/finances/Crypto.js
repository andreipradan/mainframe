...
const Crypto = () => {
  ...
  const cryptoApi = new FinanceApi("crypto", token)
  ...
  const handleTransactionsSubmit = async (event) => {
    ...
    dispatch(cryptoApi.upload(token, formData))
  ...
  const onTickerChange = (newValue, store = crypto) => {
    const setKwargsMethod = store === crypto ? setKwargs : setPnlKwargs;
    const newTypes = newValue.map(v => v.value)
  ...
  // useEffect(() => setUploadPnlOpen(Boolean(crypto.errors)), [crypto.errors]);
                    ...
                    </h6>
                    {
                      Object.keys(crypto.kwargs).length
                        ? <small className="text-warning">&nbsp;(
                          {
                            crypto.currencies?.map(c => `${c === 'USD' ? '$' : '€'} ${crypto.aggregations?.current[`total_${c}`]}`).filter(i => !i.includes(' null')).join(' | ')
                          })</small>
                        : null
                    }
                  </div>
                    ...
                    >
                      <Form.Group className="col-md-4">
                        ...
                        <Select
                          ...
                          isMulti
                          onChange={e => onCurrencyChange(e, crypto)}
                      ...
                      <Form.Group className="col-md-4">
                        ...
                        <Select
                          ...
                          isLoading={crypto.loading}
                          isMulti
                          onChange={e => onTypeChange(e, crypto)}
                      ...
                      <Form.Group className="col-md-4">
                        ...
                        <Select
                          ...
                          isLoading={crypto.loading}
                          isMulti
                          onChange={(e) => onTickerChange(e, crypto)}
                          options={crypto.tickers?.map(t => ({ label: t, value: t }))}
                          styles={selectStyles}
                    ...
                    : crypto.results?.length
                      ? crypto.results.map(transaction => <tr key={transaction.id}>
                        ...
                        <td style={{ cursor: "pointer" }}
                            onClick={() => onTypeChange([{ value: crypto.types.find(t => t[1] === transaction.type)[0] }])}> {transaction.type} </td>
                        <td> {parseFloat(transaction.quantity)} </td>
                        <td style={{ cursor: "pointer" }}
                            onClick={() => onTickerChange([{ value: transaction.symbol }])}> {transaction.symbol} </td>
