...
import {
  selectItem as selectTransaction,
  setKwargs,
...
const getColor = (type, border = false) => {
  switch (type) {
    case 'cash':
      return `rgba(255,245,64,${border ? 1 : 0.2})`;
    case 'health':
      return `rgba(75,192,126,${border ? 1 : 0.2})`;
    case 'restaurants':
      return `rgba(255,198,0,${border ? 1 : 0.2})`;
    case 'savings':
      return `rgba(54,162,235, ${border ? 1 : 0.2})`;
    case 'shopping':
      return `rgba(153,102,255,${border ? 1 : 0.2})`;
    case 'transport':
      return `rgba(75,192,126,${border ? 1 : 0.2})`;
    case 'travel':
      return `rgba(54,162,235, ${border ? 1 : 0.2})`;
    case 'utilities':
      return `rgba(255,159,64,${border ? 1 : 0.2})`;
    case 'Unidentified':
      return `rgba(255,255,255,${border ? 0.2 : 0.1})`;
    default:
      return random_rgba();
  }
};
...
const Transactions = () => {
  ...
  const [specificCategoriesModalOpen, setSpecificCategoriesModalOpen] =
    useState(false);
  const [transactionToRemove, setTransactionToRemove] = useState(null);
  const [uploadOpen, setUploadOpen] = useState(false);

  const getSpecificCategory = (description) =>
    checkedCategories?.find((c) => c.description === description)?.category;
  const paymentsData = {
      ...
      callbacks: {
        title: (tooltipItem, data) => {
          ...
          const all = data.datasets
            .map((d) => d.data[item.index])
            .reduce((partialSum, p) => partialSum + parseFloat(p), 0);
          const moneyIn = data.datasets
            .filter((d) => parseFloat(d.data[item.index]) > 0)
            .map((d) => d.data[item.index])
            .reduce((partialSum, p) => partialSum + parseFloat(p), 0);
          const moneyOut = data.datasets
            .filter((d) => parseFloat(d.data[item.index]) < 0)
            .map((d) => d.data[item.index])
            .reduce((partialSum, p) => partialSum + parseFloat(p), 0);
  ...
  useEffect(() => {
    Boolean(transactions.msg) &&
      toast.success(`${transactions.msg.message}`, toastParams);
  }, [transactions.msg]);
  useEffect(() => {
    !allChecked
      ...
      : setCheckedCategories(
          transactions.results
            ?.filter((t) => Boolean(t.category_suggestion))
            .map((t) => ({
              description: t.description,
              category: t.category_suggestion,
            }))
        );
  }, [allChecked]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (
      accounts.selectedItem &&
      selectedDate &&
      selectedDate.getFullYear() !== transactions.kwargs.year
    ) {
      dispatch(
        FinanceApi.getExpenses(
          token,
  ...
  const handleGetElementAtEvent = (element) => {
    ...
    const month =
      new Date(
        Date.parse(
          `${element[0]._model.label} 1, ${selectedDate.getFullYear()}`
        )
      ).getMonth() + 1;
  ...
  const handleSubmit = async (event) => {
    ...
    dispatch(
      TransactionsApi.uploadTransactions(token, formData, {
        account_id: accounts.selectedItem.id,
        year: selectedDate.getFullYear(),
      })
    );
  ...
  return (
    <div>
          ...
          {!accounts.selectedItem ? (
            ...
            <>
                ...
                <Dropdown.Menu>
                  {accounts.results?.map((acc) => (
                    <Dropdown.Item
                      ...
                      active={acc.id === accounts.selectedItem.id}
                    >
              ...
              <div className='btn'>
                ...
                />
                <i
                  className='mdi mdi-plus cursor-pointer'
                  onClick={() => dispatch(setModalOpen('new'))}
                />
              </div>
            </>
          )}
            ...
            <div className='card-body'>
              <h5>
                ...
                <button
                  ...
                  onClick={() =>
                    dispatch(AccountsApi.get(token, accounts.selectedItem.id))
                  }
                >
              ...
              {accounts.loading ? (
                ...
                <>
                  <ListItem
                    label={'Account'}
                    value={`${accounts.selectedItem.bank} (${accounts.selectedItem.type})`}
                    textType={'primary'}
                  />
                  <ListItem
                    label={'Number'}
                    value={accounts.selectedItem.number}
                  />
                  <ListItem
                    label={'Transactions'}
                    value={accounts.analytics?.total}
                  />
                </>
              ) : (
                '-'
            ...
            <div className='card-body'>
              ...
              {accounts.loading ? (
                ...
                <>
                  <ListItem
                    label={'First Name'}
                    value={accounts.selectedItem.first_name}
                  />
                  <ListItem
                    label={'Last Name'}
                    value={accounts.selectedItem.last_name}
                  />
                  <ListItem
                    label={'Client Code'}
                    value={accounts.selectedItem.client_code}
                  />
                </>
              ) : (
                '-'
            ...
            <div className='card-body'>
              ...
              {transactions.loading ? (
                ...
                <Marquee duration={10000} pauseOnHover>
                  <ListItem
                    label={'Completed'}
                    value={transactions.results[0].completed_at}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Started'}
                    value={transactions.results[0].started_at}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Amount'}
                    value={transactions.results[0].amount}
                    textType={
                      parseFloat(transactions.results[0]) < 0
                        ? 'success'
                        : 'warning'
                    }
                    className='mr-3'
                  ...
                  <ListItem
                    label={'Fee'}
                    value={transactions.results[0].fee}
                    textType={
                      parseFloat(transactions.results[0]) > 0 ? 'danger' : ''
                    }
                    className='mr-3'
                  />
                  <ListItem
                    label={'State'}
                    value={transactions.results[0].state}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Description'}
                    value={transactions.results[0].description}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Type'}
                    value={transactions.results[0].type}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Category'}
                    value={capitalize(transactions.results[0].category).replace(
                      '-',
                      ' '
                    )}
                    className='mr-3'
                  ...
                  <ListItem
                    label={'Product'}
                    value={transactions.results[0].product}
                    className='mr-3'
                  />
                </Marquee>
              ) : (
          ...
          <div className='card'>
            <div className='card-body'>
              <h4 className='card-title'>
                ...
                {accounts.analytics ? (
                  <>
                    {accounts.analytics.years.find(
                      (y) => y === selectedDate.getFullYear() - 1
                    ) && (
                      <button
                        type='button'
                        ...
                        onClick={() =>
                          setSelectedDate(
                            new Date(`${selectedDate.getFullYear() - 1}-01-01`)
                          )
                        }
                      >
                    ...
                    <DatePicker
                      ...
                      includeDates={accounts.analytics.years.map(
              ...
              <h4 className='card-title'>
                ...
                <div className='mb-0 text-muted'>
                  <small>
                    ...
                    {transactions.page_amount
                      ? ` | Amount: ${transactions.page_amount}`
                      : null}
                  </small>
                  <button
                    type='button'
                    ...
                    <input
                      ...
                      onChange={() =>
                        dispatch(
                          setKwargs({
                            only_expenses: !(
                              transactions.kwargs?.only_expenses || false
                            ),
                            page: 1,
                          })
                        )
                  ...
                  <li className='nav-item w-100'>
                    <form
                      ...
                      onSubmit={(e) => {
                        e.preventDefault();
                        dispatch(
                          setKwargs({ search_term: searchTerm, page: 1 })
                        );
                      }}
                    >
                      <input
                    ...
                    {transactions.loading ? (
                      ...
                      transactions.results.map((t, i) => (
                        <tr
                          ...
                          style={{
                            cursor: `${t.category_suggestion ? 'default' : 'not-allowed'}`,
                          }}
                        >
                          <td>
                          ...
                          >
                            <Select
                              ...
                              onChange={(newValue) =>
                                onCheckedCategoryChange(newValue, t.description)
                              }
                              options={transactions.categories?.map((c) => ({
                                label: getCategoryVerbose(c),
                                value: c,
                              }))}
                              styles={selectStyles}
                              value={{
                            ...
                            <p className={'text-warning ml-2'}>
                              {t.category_suggestion
                                ? capitalize(t.category_suggestion).replace(
                                    '-',
                                    ' '
                                  )
                                : null}
                            ...
                            {getSpecificCategory(t.description) && (
                              <a
                                href={'!#'}
                                onClick={(e) => {
                                  const currentCategory = getSpecificCategory(
                                    t.description
                                  );
                                  e.preventDefault();
                                  setCheckedCategories(
              ...
              >
                <Form.Group>
                  ...
                  <Select
                    ...
                    options={transactions.confirmed_by_choices?.map((c) => ({
                      label: c[1],
                      value: c[0],
                    }))}
                    styles={selectStyles}
                    value={{
                      label:
                        transactions.confirmed_by_choices?.find(
                          (c) => c[0] === transactions.kwargs.confirmed_by
                        )?.[1] || 'All',
                ...
                <Form.Group>
                  ...
                  <Select
                    ...
                    value={{
                      label:
                        getCategoryVerbose(transactions.kwargs.category) ||
                        'All',
                ...
                <Form.Group>
                  ...
                  <Select
                    ...
                    options={transactions.types?.map((t) => ({
                      label: getTypeLabel(t),
                      value: t,
                    }))}
                    styles={selectStyles}
                    placeholder={'All'}
                    value={transactions.kwargs.types?.map((t) => ({
                      label: getTypeLabel(t),
                      value: t,
                    }))}
                  />
      ...
      <Modal
        centered
        show={Boolean(transactionToRemove)}
        onHide={() => setTransactionToRemove(null)}
      >
        <Modal.Header closeButton>
          <Modal.Title>
            ...
            <ul className='text-muted mb-0'>
              <li>
                Created:{' '}
                {new Date(transactionToRemove?.created_at).toLocaleDateString()}
                &nbsp;
                <small>
                  {new Date(
                    transactionToRemove?.created_at
                  ).toLocaleTimeString()}
                </small>
              </li>
              <li>
                Started:{' '}
                {new Date(transactionToRemove?.started_at).toLocaleDateString()}
                &nbsp;
                <small>
                  {new Date(
                    transactionToRemove?.started_at
                  ).toLocaleTimeString()}
                </small>
              </li>
              <li>
                Amount: {transactionToRemove?.amount}{' '}
                {parseFloat(transactionToRemove?.fee)
                  ? `(Fee: ${transactionToRemove?.fee})`
                  : ''}
              </li>
        ...
        {(transactions.errors?.detail || transactions.errors?.length) &&
        transactionToRemove ? (
          <Errors errors={transactions.errors} />
        ) : null}
