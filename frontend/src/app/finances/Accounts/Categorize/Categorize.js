...
const Categorize = () => {
  ...
  const unpredictedCategories = transactions.results
    ?.filter((t) => !t.category_suggestion)
    ?.map((t) => t.description);
  ...
  useEffect(() => {
    setMessageAlertOpen(!!transactions.msg);
  }, [transactions.msg]);
  useEffect(() => {
    !allChecked
  ...
  useEffect(() => {
    ...
    if (['executing', 'initial'].includes(prediction.train?.status))
      startPolling();
    else stopPolling();
  ...
  useEffect(() => {
    ...
    if (
      ['executing', 'initial', 'progress'].includes(prediction.predict?.status)
    )
      startPolling();
    else stopPolling();
  ...
  return (
    <div>
      <div className='page-header'>
        <h3 className='page-title'>
          ...
          <div className='mb-0 text-muted'>
            <small>Train and improve the auto detection engine</small>
          </div>
        </h3>
          ...
          <div className='card'>
            <div className='card-body'>
              <h4
                ...
                onClick={() => setPredictionTasksOpen(!predictionTasksOpen)}
              ...
              >
                ...
                {predictionTasksOpen ? null : (
                  <small className='small text-muted'>Click to expand</small>
                )}
              </h4>
                      ...
                      <tr>
                        ...
                        {prediction.train ? (
                          <>
                            <td>
                              {new Date(
                                prediction.train.history[0].timestamp
                              ).toLocaleString()}
                            </td>
                            <td>
                            ...
                            <td>
                              {prediction.train.accuracy ? (
                                <span>
                                  Accuracy:&nbsp;
                                  {(
                                    parseFloat(prediction.train.accuracy) * 100
                                  ).toFixed(1)}
                                  %
                                </span>
                              ) : null}
                              {prediction.train.count ? (
                                <>
                                  <br />
                                  Training objects count:{' '}
                                  {prediction.train.count}
                                </>
                              ) : null}
                              ...
                              >
                                History{' '}
                                <i
                                  className={`mdi mdi-chevron-${trainHistoryOpen ? 'down' : 'right'}`}
                                />
                              </a>
                            ...
                            <td>
                              {prediction.loadingTrain ? (
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    dispatch(
                                      PredictionApi.getTask(token, 'train')
                                    )
                                  }
                                >
                                  <i className='mdi mdi-refresh' />
                      ...
                      <tr>
                        ...
                        {prediction.predict ? (
                          <>
                            <td>
                              {new Date(
                                prediction.predict.history[0].timestamp
                              ).toLocaleString()}
                            </td>
                            <td>
                            ...
                            <td>
                              {prediction.predict.operation ? (
                                <>
                                  <br />
                                  <i>{prediction.predict.operation}</i>
                                </>
                              ) : null}
                              {prediction.predict.progress ? (
                                <>
                                  <br />
                                  Progress {prediction.predict.progress}%
                                </>
                              ) : null}
                              {prediction.predict.error ? (
                                <>
                                  <br />
                                  Error: {prediction.predict.error}
                                </>
                              ) : null}
                              <br />
                              <a
                              ...
                              >
                                History{' '}
                                <i
                                  className={`mdi mdi-chevron-${predictHistoryOpen ? 'down' : 'right'}`}
                                />
                              </a>
                            ...
                            <td>
                              {prediction.loadingPredict ? (
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    dispatch(
                                      PredictionApi.getTask(token, 'predict')
                                    )
                                  }
                                >
                                  <i className='mdi mdi-refresh' />
          ...
          <div className='card'>
            <div className='card-body'>
              <h4 className='card-title'>
                ...
                <div className='mb-0 text-muted'>
                  <small>
                    Total: {transactions.count} | Unidentified:{' '}
                    {transactions.unidentified_count}
                  </small>
                  <button
                    type='button'
                  ...
                  <li className='nav-item w-100'>
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          dispatch(setKwargs({ search_term: e.target.value }))
                        }
                      />
                    </form>
                  </li>
              ...
              >
                <Form.Group>
                  ...
                  <Select
                    ...
                    styles={selectStyles}
                    value={{
                      label: transactions.confirmed_by_choices?.find(
                        (c) => c[0] === kwargs.confirmed_by
                      )?.[1],
                ...
                <Form.Group>
                  ...
                  <Select
                    ...
                    options={transactions.accounts?.map((account) => ({
                      label: `${account.bank} - ${account.type}`,
                      value: account.id,
                    }))}
                    styles={selectStyles}
                    value={{
                      label: transactions.accounts?.find(
                        (c) => c.id === kwargs.account_id
                      )?.bank,
      ...
      <Modal
        centered
        show={trainingModalOpen}
        onHide={() => setTrainingModalOpen(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title>
            ...
            <p className='text-muted mb-0'>
              Train machine learning model on current categories
            </p>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <p>
            e.g. descriptions that might not always belong to the same category
          </p>
          Proceed?
        </Modal.Body>
        <Modal.Footer>
          <Button variant='success' onClick={() => setTrainingModalOpen(false)}>
            No, go back
          </Button>
          <Button
            variant='danger'
      ...
      <Modal
        centered
        show={specificCategoriesModalOpen}
        onHide={() => setSpecificCategoriesModalOpen(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title>
            <div className='row'>
        ...
        <Modal.Body>
          ...
          <ul>
            {checkedCategories?.map((c, i) => (
              <li key={i}>
                {c.description}
                <span className='text-warning'> -> New category ->&nbsp;</span>
        ...
        <Modal.Footer>
          <Button
            variant='success'
            onClick={() => setSpecificCategoriesModalOpen(false)}
          >
            No, go back
          </Button>
          <Button
            ...
            onClick={() => {
              dispatch(
                TransactionsApi.bulkUpdateTransactions(
                  token,
                  checkedCategories,
                  kwargs
                )
      ...
      <Modal
        centered
        show={predictModalOpen}
        onHide={() => setPredictModalOpen(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title>
            <div className='row'>
            ...
            <p className='text-muted mb-0'>
              Predict categories for {checkedCategories?.length || 'all the'}{' '}
              transactions
            </p>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p className='mb-0'>
            This will start predicting the categories for{' '}
            <span className='text-danger'>
              {' '}
              {checkedCategories?.length || 'all the'} transactions
            </span>
          </p>
          <p className='mb-0'>
            Depending on the number it might take a while so please be patient
          </p>
          Proceed?
        </Modal.Body>
        <Modal.Footer>
          <Button variant='success' onClick={() => setPredictModalOpen(false)}>
            No, go back
          </Button>
          <Button
            variant='danger'
            onClick={() => {
              dispatch(
                PredictionApi.predict(
                  token,
                  checkedCategories?.map((t) => t.description) || [],
                  kwargs
                )
              );
