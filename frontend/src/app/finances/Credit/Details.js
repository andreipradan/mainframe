...
const Details = () => {
  ...
  const onChangeCurrency = (newValue) => {
    ...
    const totalRemaining =
      principalRemaining + interestRemaining + insuranceRemaining;
    ...
    setSummaryInterest(
      getAmountInCurrency(interestPaid + interestRemaining, currency)
    );
    ...
    setPaidPrepaid(
      getAmountInCurrency(overview.payment_stats?.prepaid, currency)
    );
    setPaidPrincipal(
    ...
    setBarChartInterest(
      getInterest(payment.results).map((p) => getAmountInCurrency(p, currency))
    );
  };
  ...
  const paymentsOptions = {
    ...
    tooltips: {
      callbacks: {
        label: (tooltipItem, data) => {
          const otherValue =
            data.datasets[tooltipItem.datasetIndex === 0 ? 1 : 0].data[
              tooltipItem.index
            ];
          const currentValue =
          ...
          const percentage = parseFloat(
            (
              (currentValue /
                (parseFloat(currentValue) + parseFloat(otherValue))) *
              100
            ).toFixed(1)
          );
        ...
        title: (tooltipItem, data) => {
          ...
          const otherValue = parseFloat(
            data.datasets[item.datasetIndex === 0 ? 1 : 0].data[item.index]
          );
    ...
    <div>
      ...
      <div className='page-header'>
        <h6 className='page-title'>
          ...
          selectedCurrency.label !== credit?.currency ? (
            <small className='text-warning'>
              Rate: 1 {selectedCurrency.label} ={' '}
              {rates.find((r) => r.symbol === selectedCurrency.value)?.value}{' '}
              {credit?.currency}
              <br />
              From:{' '}
              {rates.find((r) => r.symbol === selectedCurrency.value)?.date}
              <br />
              Source:{' '}
              {rates.find((r) => r.symbol === selectedCurrency.value)?.source}
            </small>
          ) : null}
        </h6>
        {rates?.length ? (
          <Select
            ...
            options={[
              { label: credit?.currency, value: credit?.currency },
              {
                label: 'EUR',
                value: rates.find(
                  (r) => r.symbol.replace(credit?.currency, '') === 'EUR'
                ).symbol,
              },
              {
                label: 'USD',
                value: rates.find(
                  (r) => r.symbol.replace(credit?.currency, '') === 'USD'
                ).symbol,
              },
              ...rates
                .map((c) => ({
                  label: c.symbol.replace(credit?.currency, ''),
                  value: c.symbol,
                }))
            ...
            <div className='card-body'>
              ...
              {overview.loading ? (
                ...
                <>
                  ...
                  <ListItem
                    label={'Months'}
                    value={`${credit.number_of_months} (${credit.number_of_months / 12} yrs)`}
                  />
                  <ListItem
                    label={'Saved'}
                    value={summarySaved}
              ...
              {payment.loading ? (
                ...
                <>
                  ...
                  <ListItem
                    ...
                    value={paidPrepaid}
                    textType={'success'}
                    tooltip='prepaid-percentage'
                  />
                  <ListItem
                    label={'Principal'}
                    value={paidPrincipal}
                    textType={'success'}
                    tooltip={'principal-percentage'}
                  />
                  <ListItem
                    label={'Interest'}
                    value={paidInterest}
                    textType={'danger'}
                    tooltip={'interest-percentage'}
                  />
                </>
              ) : (
                '-'
          ...
          <div className='card'>
            <div className='card-body'>
              <h6>
                ...
                {overview.loading ? null : latestTimetable?.length ? (
                  <span className='text-warning'>
                    {' '}
                    {latestTimetable[0].date}
                  </span>
                ) : null}
              </h6>
              {overview.loading ? (
                ...
                <Marquee pauseOnHover>
                  <ListItem
                    label={'Total'}
                    value={getAmountInCurrency(latestTimetable[0].total)}
                    textType={'primary'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Principal'}
                    value={getAmountInCurrency(latestTimetable[0].principal)}
                    textType={'success'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Interest'}
                    value={`${getAmountInCurrency(latestTimetable[0].interest)} (${overview.latest_timetable.interest}%)`}
                    textType={'danger'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'IRCC'}
                    value={`${overview.latest_timetable.ircc}%`}
                    textType={'danger'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Insurance'}
                    value={getAmountInCurrency(latestTimetable[0].insurance)}
                    textType={'danger'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Remaining'}
                    value={getAmountInCurrency(latestTimetable[0].remaining)}
                    textType={'danger'}
                    className='mr-3'
                  />
                </Marquee>
            ...
            <div className='card-body'>
              ...
              {payment.loading ? (
                ...
                <Marquee duration={10000} pauseOnHover>
                  <ListItem
                    label={'Total'}
                    value={getAmountInCurrency(payment.results[0].total)}
                    textType={'primary'}
                    className='mr-3'
                  />
                  <ListItem
                    label={'Date'}
                    value={payment.results[0].date}
                    textType={'warning'}
                    className='mr-3'
                  />
                  <ListItem
                    ...
                    textType={
                      payment.results[0].is_prepayment ? 'success' : 'warning'
                    }
                    className='mr-3'
                  />
                  <ListItem
                    label={'Principal'}
                    value={getAmountInCurrency(payment.results[0].principal)}
                    textType={'success'}
                    className='mr-3'
                  />
                  {payment.results[0].interest > 0 && (
                    <ListItem
                      label={'Interest'}
                      value={getAmountInCurrency(payment.results[0].interest)}
                      textType={'danger'}
                      className='mr-3'
                    />
                  )}
            ...
            <div className='card-body'>
              <h6 className='card-title'>
                ...
                <button
                  type={'button'}
                  className={
                    'btn btn-outline-success btn-sm border-0 bg-transparent'
                  }
      ...
      <Tooltip anchorSelect='#paid-percentage' place='bottom-start'>
        {getPercentage(paidTotal, summaryCredit)}% of the total credit
        <br />~ {getPercentage(paidTotal, remainingTotal)}% of the remaining
        total
      </Tooltip>
      <Tooltip anchorSelect='#prepaid-percentage' place='bottom-start'>
        {getPercentage(paidPrepaid, summaryCredit)}% of the total credit
        <br />
        {getPercentage(paidPrepaid, paidTotal)}% of the paid amount
      </Tooltip>
      <Tooltip anchorSelect='#principal-percentage' place='bottom-start'>
        {getPercentage(paidPrincipal, summaryCredit)}% of the total credit
        <br />
        {getPercentage(paidPrincipal, remainingPrincipal)}% of the remaining
        principal
        <br />
        {getPercentage(paidPrincipal, paidTotal)}% of the paid amount
      </Tooltip>
      <Tooltip anchorSelect='#interest-percentage' place='bottom-start'>
        {getPercentage(paidInterest, summaryCredit)}% of the total credit
        <br />~ {getPercentage(paidInterest, remainingInterest)}% of the
        remaining interest
        <br />
